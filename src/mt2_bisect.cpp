...
double mt2::get_mt2()
{
   if (!momenta_set)
   {
       cerr <<" Please set momenta first!" << endl;
       return 0;
   }
...
//look for when both parablos contain origin
   ...
   if (nsols_high == nsols_low)
   {
    ...
    cerr<<"Deltasq_high not found at event " << nevt <<endl;
   ...
   if(nsols_high == nsols_low)
   {
      cerr << "error: nsols_low=nsols_high=" << nsols_high << endl;
      cerr << "Deltasq_high=" << Deltasq_high << endl;
      cerr << "Deltasq_low= "<< Deltasq_low << endl;
...
int mt2::scan_high(double & Deltasq_high)
{
   ...
   tempmass = mn + ma;
   maxmass  = sqrt(mnsq + Deltasq_high);
   if (nevt == 32334) cerr << "Deltasq_high = " << Deltasq_high << endl;
